...
SELECT `genres`.`name` FROM `genres` LEFT JOIN `books` ON `genres`.`id` = `books`.`genre_id` WHERE `books`.`genre_id` IS NULL;
...
SELECT `users`.`surname`, `users`.`name`, `areas`.`name` FROM `areas` JOIN `users` ON `areas`.`id` = `users`.`area_id` ORDER BY `areas`.`name` LIMIT 4;

-- 3. Необходимо посчитать сколько новостей в каждой категории. Связь `news`.`category_id` = `categories`.`id`.
SELECT `categories`.`name`, COUNT(`news`.`id`) FROM `categories` JOIN `news` ON `categories`.`id` = `news`.`category_id` GROUP BY `categories`.`name`;
...
SELECT `cities`.`name`, `areas`.`name` FROM `areas` JOIN `cities` ON `areas`.`id` = `cities`.`area_id`;
...
SELECT `schools`.`name`, `districts`.`name` FROM `districts` JOIN `schools`ON `districts`.`id` = `schools`.`district_id`;